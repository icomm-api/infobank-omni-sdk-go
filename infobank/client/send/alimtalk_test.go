...
func TestAlimtalkSender_SendMessage(t *testing.T) {
	alimtalkSender := NewAlimTalkSender("input-your-token", nil)
	...
	message := NewAlimtalkBuilder().KakaoMsgType("AT").SenderKey("senderKey").To("01012345678").Text("test").TemplateCode("tmplateCode").Ref("ref").Fallback(fallback).Build()
	apiResponse, err := alimtalkSender.SendMessage(&message)
	if err != nil {
		t.Error("err : ", err)
		return
	}
